<script>
  ...
  $: document.body.className = bem( 'action-panel', { theme } ).join( " " )
  ...
  const __MSG_tab_group_manage__ = getMessage( "tab_group_manage" )
  ...
  onStateChange( state => {
    ...
    const new_selected_tab_ids = getNewSelectedTabIds( selected_tab_ids, state_window )
    const search_missed_tab_ids = []

    tab_groups = state_window.tab_groups.map( cloneTabGroup )
    // Use the extended splice to trigger change detection
    tab_groups.forEach( tab_group => {
      // Copy the `open` flag from original data
      const orig_tab_group = tab_groups.find( _tab_group => _tab_group.id === tab_group.id )
      if( orig_tab_group ) {
    ...
    // Use the extended splice to trigger change detection
    // Object.getPrototypeOf( search_missed_tab_ids ).splice.apply( search_missed_tab_ids, [ 0, search_missed_tab_ids.length, ...search_missed_tab_ids ] )
    // Object.getPrototypeOf( selected_tab_ids ).splice.apply( selected_tab_ids, [ 0, selected_tab_ids.length, ...new_selected_tab_ids ] )
    // Object.getPrototypeOf( pinned_tabs ).splice.apply( pinned_tabs, [ 0, pinned_tabs.length, ...tab_groups[ 0 ].tabs ] )
  ...
  function onTabGroupClick( tab_group ) {
    window.background.openTabGroup( window.store, window_id, tab_group.id )
    window.close()
  }
</script>
...
<style lang="sass">
  ...
  @each $theme, $colors in $action__theme {
    :global(.action-panel--theme-#{$theme}) {
...
<div class="panel">
  ...
  <div class="panel-section panel-section-list panel-section-content">
    {#each tab_groups as tab_group (tab_group.id)}
    {#if ! tab_group.pinned}
    <div class="{ bem( 'panel-list-item', { 'active': tab_group.id === active_tab_group_id } ).join( ' ' ) }">
      <div class="text" on:click={ e => onTabGroupClick( tab_group ) }>
        { tab_group.title }
      </div>
      {#if tab_group.audible}
      <svg class="{ bem( 'panel-list-item__icon', { 'audio': true } ) }" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
        <g>
      ...
      {/if}
      <div on:click={ e => onTabGroupClick( tab_group ) }>
  ...
  <div class="panel-section panel-section-footer">
    <div class="panel-section-footer-button" on:click={ e => openTabGroupsPage() }>
      <img class="icon" src="/icons/tabulate.svg" alt="Tabulate"/>
      <span class="text" v-once>{ __MSG_tab_group_manage__ }</span>
    </div>
    <div class="panel-section-footer-separator"></div>
    <div class="panel-section-footer-button panel-section-footer-button-options" on:click={ handleOpenOptionsClick }>
