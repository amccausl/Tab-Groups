<script>
  ...
  let window_id = window.current_window_id
  let search_history = []

  const __MSG_tab_search_placeholder__ = window.background.getMessage( "tab_search_placeholder" )

  onStateChange( state => {
    const state_window = getWindow( state, window_id )
    if( state_window && state_window.search != null ) {
      const index = search_history.indexOf( state_window.search.text )
      if( index > -1 ) {
        search_history.splice( index, 1 )
      } else {
        search_text = state_window.search.text
      }
  ...
  function onUpdateSearchText() {
    ...
    search_history.push( search_text )
    window.background.runTabSearch( window.store, window_id, search_text )
  }

  function clearSearchText() {
    console.info('clearSearchText', search_text)
    search_text = ""
    window.background.runTabSearch( window.store, window_id, search_text )
  }
</script>
...
<div class="{ bem( `tab-search`, { theme } ).join( ' ' ) }">
  <label class="tab-search__label">
    ...
    <input class="tab-search__input" type="search" bind:value={ search_text } on:input={ e => onUpdateSearchText() } placeholder={ __MSG_tab_search_placeholder__ }/>
    <svg class="tab-search__clear-icon" on:click="{ e => clearSearchText() }" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
